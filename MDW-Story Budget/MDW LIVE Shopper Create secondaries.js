...
for (let field of slugTable.fields) {
    ...
    if (
        ...
        if (field.name == "Secondary Publication") {
            ...
            // console.log(field.name + `: newRecord value `+thisRecord.getCellValue(field.name));
...
for (let recordToCreate of recordsToCreate) {
    ...
    if (secondaryPubs) {
        if (secondaries) {
            ...
            OUTER: for (let secondaryPub of secondaryPubs) {
                //de duplication
        ...
        for (let secondaryPub of secondaryPubs) {
            ...
            if (secondaryPub.name == 'All Midwest/Communities') {
                ...
                for (let publicationTitle of publicationTitles.records) {
                    if (publicationTitle.name != 'Wires' && publicationTitle.name != 'All Midwest/Communities' && publicationTitle.name != recordToCreate.fields['Primary Publication'][0].name ) {
        ...
        if (secondaryPubs) {
            ...
            for (let secondaryPub of secondaryPubs) {
                ...
                if (debug == "false") {
//this can be refactored to push once...                    
