...
console.log(`${inputConfig.txComUpdate}`);
...
if (`${inputConfig.lastModifiedBy}` != 'Automations' || 
    (`${inputConfig.lastModifiedBy}` == 'Automations' && `${inputConfig.txComUpdate}` == "true") ) {
    let updateFields = {
        thisChannelID: { 
                field:'Channel ID',
                type: 'selectById'
...
//this can be refactored to push one time instead of multiple...
        ...
        table.updateRecordAsync(`${inputConfig.thisAirtableId}`, {'TX-Com update': false});
    } else {
        //secondary record
