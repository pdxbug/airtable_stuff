...
//javascript push of array will overwrite all array entries with the latest push
//due to something about using a referenced array on push. This clone function
//will make sure to iterate and just add the new array entry
    ...
    if (secondaryPubs) {
        ...
        for (let secondaryPub of secondaryPubs) {
            ...
            if (secondaryPub.name == 'All CT dailies') {
                ...
                for (let publicationTitle of publicationTitles.records) {
                    if (
                        publicationTitle.name != 'Wires' && 
                        publicationTitle.name != 'All CT dailies' && 
                        publicationTitle.name != recordToCreate.fields['Primary Publication'][0].name 
                    ) {
        ...
        if (secondaryPubs) {
            ...
            for (let secondaryPub of secondaryPubs) {
                ...
                //we need to create one createRecordsAsync array and push once
                //but need to retrieve the secondary IDs to update the primary record
                secondaryRecords.push( {fields: clone(recordToCreate.fields)} );
            }
            ...
            if (secondaryRecords.length > 0) {
                ...
                if (debug == "false") {
                    let createdSecondaryIds = await slugTable.createRecordsAsync(secondaryRecords);
                    console.log('created secondary');
                    console.log(createdSecondaryIds);
                    for (let createdSecondaryId of createdSecondaryIds) {
                        secondaryIds.push({id: createdSecondaryId});
                    }
                } else {
                ...
                if (secondaryIds.length > 0) {
                    ...
                    let thisSecondaryRecords = thisRecord.getCellValue("Secondary Records");
                    console.log('primaryRecord');
                    console.log(thisRecord.id);
                    // console.log(secondaryRecords);

                    if (thisSecondaryRecords != null) {
                        for (let secondaryRecord of thisSecondaryRecords) {
