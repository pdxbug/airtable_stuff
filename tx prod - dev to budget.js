...
if (newRecords.records.length == 0) {
    ...
    for (let newRecord of newRecords.records) {
        ...
        let records = '';
        try {
            records = queryResult.getRecord(newRecord.id);
        }
        catch(err) {
            console.log('Most likely this id was caught on a previous automation from multiple stories being sent at the same time');
